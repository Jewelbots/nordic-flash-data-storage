...
#define JWB_NUM_OF_FRIENDS_INSTANCE_ID 	0x2222
#define JWB_NUM_OF_FRIENDS_TYPE_ID 		  0x1111
...

{
    switch(cmd)
    {
        ...
        case FDS_CMD_UPDATE:
						SEGGER_RTT_WriteString(0, "FDS_CMD_UPDATE\n");
        ...
        default:
					SEGGER_RTT_printf(0, "default command is: %u\n", cmd);
            break;
    }
}
void save_number_of_friends(uint32_t * num_of_friends) {
	...
	key.type = JWB_NUM_OF_FRIENDS_TYPE_ID;
	...
	uint32_t err_code = fds_write(&num_of_friends_descriptor, key, 1, &chunk);
...
void load_number_of_friends(uint32_t * number_to_load) {
	...
	fds_record_t record;
	uint32_t data = 0;
	while (fds_find(JWB_NUM_OF_FRIENDS_TYPE_ID, JWB_NUM_OF_FRIENDS_INSTANCE_ID, &descriptor, &tok) == NRF_SUCCESS) {
		...
		*number_to_load = (uint32_t)(*record.p_data);
		...
		SEGGER_RTT_printf(0, "loaded data was: %p\n", data);
